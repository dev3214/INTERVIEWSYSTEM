...
const adminItems = [
  ...
  { title: "Colleges", url: "/admin/colleges", icon: Building2 },
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      <div className="p-4 lg:p-6 border-b border-gray-200">
        ...
        {/* Show college context for candidates */}
        {user?.role === 'candidate' && user?.collegeSlug && (
          <div className="mt-2 p-2 bg-blue-50 border border-blue-200 rounded-md">
            <p className="text-xs text-blue-800 font-medium">🎓 {user.collegeSlug.replace('-', ' ').toUpperCase()}</p>
            <p className="text-xs text-blue-600">College Portal</p>
          </div>
        )}
      </div>
      ...
      <div className="border-t border-gray-200 p-3 lg:p-4">
        <div className="flex items-center">
          ...
          <Button 
            ...
            size="sm" 
            onClick={onLogout} 
            className="ml-2 p-1 lg:p-2" 
            title={user?.role === 'candidate' && user?.collegeSlug 
              ? `Logout (will return to ${user.collegeSlug.replace('-', ' ').toUpperCase()} portal)` 
              : "Logout"
            }
          >
...
export function AppSidebar({ children }: { children: React.ReactNode }) {
  ...
  const logout = async () => {
    try {
      // Check if user has college context from session
      if (user?.collegeSlug) {
        // Redirect to college-specific login page
        await signOut({ callbackUrl: `/login/${user.collegeSlug}` })
      } else {
        // Fallback: Check sessionStorage for college context
        const collegeContext = sessionStorage.getItem('collegeContext')
        if (collegeContext) {
          try {
            const context = JSON.parse(collegeContext)
            await signOut({ callbackUrl: `/login/${context.collegeSlug}` })
            // Clear sessionStorage after logout
            sessionStorage.removeItem('collegeContext')
            return
          } catch (error) {
            console.error("Error parsing college context:", error)
          }
        }
