...
import { Calendar, Clock, Play, CheckCircle, AlertCircle, FileText, Code, Brain, Users, Bell, Edit } from "lucide-react"
...
export default function CandidateInterviewsPage() {
  ...
  useEffect(() => {
    async function fetchRounds() {
      ...
      try {
        ...
        setSelectedDomain(workDomain || "")

        // Fetch available domains
        try {
          const domainsRes = await axios.get("/api/candidate/get-college-domains")
  ...
  const handleUpdateDomain = async () => {
    if (!selectedDomain) {
      toast.error("Please select a work domain")
      return
    }

    setUpdatingDomain(true)
    try {
      const selectedDomainObj = domains.find(d => d.domainname === selectedDomain)
      if (!selectedDomainObj) {
        toast.error("Selected domain not found")
        return
      }

      const response = await axios.put("/api/candidate/update-work-domain", {
        workDomain: {
          id: selectedDomainObj._id,
          name: selectedDomainObj.domainname
        }
      })

      if (response.data.success) {
        toast.success("Work domain updated successfully!")
        setDomainName(selectedDomain)
        setIsEditingDomain(false)
        
        // Refresh rounds for the new domain
        const domainRes = await axios.get(`/api/rounds?domainname=${encodeURIComponent(selectedDomain)}`)
        setDomainRounds(domainRes.data.rounds || [])
      } else {
        toast.error("Failed to update work domain")
      }
    } catch (error) {
  ...
  return (
    <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
      ...
      {/* Instruction boxes - only show if domain is selected */}
      {domainName && (
        <div className="mb-4">
      ...
      <Card className="mb-6">
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-xl">Work Domain</CardTitle>
              <CardDescription>
                Select your preferred work domain to see relevant interview rounds
              </CardDescription>
            </div>
            {!isEditingDomain && (
              <Button
          ...
          {isEditingDomain ? (
            <div className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="work-domain">Select Work Domain</Label>
                <Select value={selectedDomain} onValueChange={setSelectedDomain}>
                  <SelectTrigger>
                    <SelectValue placeholder="Choose a work domain" />
                  </SelectTrigger>
                  <SelectContent>
                    {domains.map((domain) => (
                      <SelectItem key={domain._id} value={domain.domainname}>
                        {domain.domainname}
                      </SelectItem>
                    ))}
                  </SelectContent>
            ...
            <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
              <div>
                ...
                <p className="text-lg font-semibold text-blue-600">
                  {domainName || "No domain selected"}
                </p>
              </div>
              {domainName && (
                <Badge variant="secondary" className="text-sm">
                  {domainRounds.length} rounds available
                </Badge>
              )}
            </div>
          )}
      ...
      {/* Progress Overview - only show if domain is selected */}
      {domainName && (
        <>
