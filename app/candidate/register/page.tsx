...
import { Upload, User, Briefcase, GraduationCap, FileText, CheckCircle, Loader2 } from "lucide-react"
...
export default function CandidateRegisterPage() {
  ...
  const [availableDomains, setAvailableDomains] = useState(fallbackDomains)
  ...
  // Fetch domains based on college context
  useEffect(() => {
    fetchDomains()
  }, [session])
  ...
  const fetchDomains = async () => {
    try {
      ...
      // Check if user has college context
      const collegeId = session?.user?.collegeId
      
      let response
      if (collegeId) {
        // Fetch college-specific domains
        response = await axios.get(`/api/domains/college?collegeId=${collegeId}`)
      } else {
        // Fetch all domains (for non-college users)
        response = await axios.get("/api/domains")
      }
      
      if (response.data && response.data.domains) {
        // Transform the domains to match the expected format
        const transformedDomains = response.data.domains.map((domain: any) => ({
          value: domain._id,
          label: domain.domainname,
          description: domain.description || `${domain.domainname} development`
        }))
        setAvailableDomains(transformedDomains)
      }
    } catch (error) {
      console.error("Failed to load domains:", error)
      // Keep fallback domains if API fails
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          >
            <SelectTrigger>
              <SelectValue placeholder={isLoadingDomains ? "Loading domains..." : "Select your preferred domain"} />
            </SelectTrigger>
            <SelectContent>
              {isLoadingDomains ? (
          ...
          {/* Show college context message if user has college context */}
          {session?.user?.collegeId && !isLoadingDomains && (
            <div className="flex items-center gap-2 mt-1 p-2 bg-blue-50 border border-blue-200 rounded text-blue-800 text-xs">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              <span>Showing domains available for your college</span>
            </div>
          )}
        </div>
